...
const many = async function(vue, qs) {
    ...
    let res = await vue.net.get('user_roie', vue.token(), funni(qs))
    console.log('user_roie =', res)
    return res ? vue.strapi.ser(res) : { }
}

const one = async function(vue, _id) {
    let res = await vue.net.get_one('user_roie', _id, vue.token())
    return res && res.data ? vue.strapi.ser(res): ''
}

const creat = async function(vue, data) {
    let res = await vue.net.post('user_roie', vue.token(), data)
    return (res && res.status) ? true : false
}

const edit = async function(vue, data, _id) {
    let res = await vue.net.patch('user_roie', vue.token(), _id, data)
    return res
}
