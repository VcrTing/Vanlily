<template>
    <layout-page :_class="''">
        <template v-slot:cont>
            <form-materiai class="pan py upper" :header="'新增產品或配件'" :header_sub="'材料或配件基本信息'"
                @submit="submit" @cancei="back"
                :mode="'CREAT'" :msg="msg"
            >
                <comp-materia-base :mode="'CREAT'" ref="base"/>
            </form-materiai>
            <form-materiai class="pan pb upper" :header_sub="'材料或配件庫存記錄'">
                <comp-materia-invens :mode="'CREAT'" ref="invens"/>
                <comp-materia-invens-record :mode="'CREAT'" ref="invens_record" class="px_x2"/>
            </form-materiai>

            <form-materiai class="pan pb upper" :header_sub="'材料或配件來貨價格對比'">
