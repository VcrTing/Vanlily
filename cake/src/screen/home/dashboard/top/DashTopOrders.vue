<template>
    <div class="dash-top-card">
        ...
        <nav class="fx-s">
            <div class="h0">
                <fk-countup :v="one.orderCount" v-if="one.orderCount"/>
                <span v-else>0</span>
            </div>
...
export default {
  components: { FkCountup },
    ...
    computed: {
        ...
        num() {
            const res = this.one.pius ? Math.abs( this.one.pius.orderCount ) : 0
            return this.one.orderCount ? res : 0
        }
    }
}
</script>