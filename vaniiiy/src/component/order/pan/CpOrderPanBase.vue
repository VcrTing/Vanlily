<template>
        <div class="f_row">
            <div class="w-36 f-area pr_x2">
                ...
                <nav class="pt">
                    <p>生日牌內容：<span>{{ cake_special_needs ? cake_special_needs : '(无)' }}</span></p>
                    <p>數量：<span>{{ quantity }}</span></p>
                </nav>
            </div>
            <div class="w-40 f-area pr_x2">
                <div>備註一：<span>{{ order.remarks_1 }}</span></div>
                ...
                <p class="pt">應付總額：
                    <span>
                        <view-money v-if="order.payment_fee" :money="order.payment_fee"/><span class="" v-else>(暂无数据)&nbsp;</span>
